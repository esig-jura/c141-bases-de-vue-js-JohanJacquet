  ...
  <v-container max-width="700">
    ...
    <div class="exe-zone">
      ...
      <v-card class="mx-auto my-6 pa-2" max-width="400">
        ...
        <v-card-subtitle :class="isHighlighted ? 'highlight' : 'notHighlight'">
          {{ description }}
        </v-card-subtitle>

        <!-- Rendez ce bouton fonctionnel pour changer la couleur -->
        <v-card-actions>
          <v-btn color="primary" @click="btnIsHighlightedClicked">
            {{ btnText }}
          </v-btn>
        </v-card-actions>
      </v-card>
...
const title = ref("Bienvenue à l'exercice 1");
const description = ref("Cet exercice teste le rendu déclaratif et les liaisons d'attributs.");
const isHighlighted = ref(false);
const btnText = ref("btnIsHighlightedClicked")

function btnIsHighlightedClicked() {
  if (isHighlighted.value) {
    btnText.value = "Mettre en surbrillance"
  } else {
    btnText.value = "Réinitialiser la couleur"
  }
